...
`;
        try {
            ...
            // Basic validation of adjusted text length
            ...
            return { title: originalTitle, text: originalText }; // Return original on error
        }
    }
    ...
    async function checkAiScore(text) {
        ...
        try {
            ...
            const score = parseInt(scoreResponse.match(/\d+/)?.[0] || '50', 10); // Extract digits only
            return score;
        } catch (error) {
    ...
    async function generateAndAdaptEssay(essayInfo) {
        ...
        showNotification('Gerando redação...', 'info', 0); // Persistent initial notification
        ...
        try {
            ...
            if (!rawEssay || !rawEssay.includes('TÍTULO:') || !rawEssay.includes('TEXTO:')) {
                // Attempt basic cleanup if format is wrong
                ...
                return { title: fallbackTitle, text: rawEssay, aiScore: 90 }; // Assume high AI score for fallback
            }
            ...
            // Format title
            essayTitle = essayTitle.split(' ')
                .map((word, index) => index === 0 ? word.charAt(0).toUpperCase() + word.slice(1).toLowerCase() : word.toLowerCase())
                .join(' ');

            // Basic length checks (AI should handle this via prompt, but good fallback)
            ...
            if (chars < HUMAN_WRITER_PRO.MIN_CHARS || lines < HUMAN_WRITER_PRO.MIN_LINES) {
                ...
                essayText = essayText.substring(0, essayText.lastIndexOf('.') + 1); // Cut at last sentence end
            }

            // Check AI score and adjust if needed
            let aiScore = await checkAiScore(essayText);
            if (aiScore > 55) { // Adjustment threshold
                ...
                // Optionally re-check score: aiScore = await checkAiScore(essayText);
            } else {
    ...
    async function actionGenerateEssay() {
        ...
        if (hckButton) hckButton.disabled = true; // Disable button during generation

        try {
            // Basic check if it's an essay page (adapt selector if needed)
            const activityElement = document.querySelector('p.MuiTypography-root.MuiTypography-body1'); // Example selector
            ...
            const essayInfo = {
                coletanea: document.querySelector('.coletanea-container')?.innerText || // Example selectors
                           document.querySelector('[data-testid="coletanea"]')?.innerText ||
                           document.querySelector('.css-1pvvm3t')?.innerText || // Original selector
                           '',
                enunciado: document.querySelector('.enunciado-container')?.innerText || // Example
                           document.querySelector('.ql-editor')?.innerText || // Common editor
                           document.querySelector('.css-1cq7p20')?.innerText || // Original
                           '',
                generoTextual: document.querySelector('.genero-textual')?.innerText || 'dissertativo-argumentativo', // Example
                criteriosAvaliacao: document.querySelector('.criterios-avaliacao')?.innerText || // Example
                                   document.querySelector('.css-kf35ou .ql-editor')?.innerHTML || // Original
                                   ''
            };
            ...
            // Generate and adapt essay
            const { title, text, aiScore } = await generateAndAdaptEssay(essayInfo);

            // Find textareas (usually title first, essay last)
            ...
            if (!textSuccess) {
                showNotification('Falha ao inserir texto.', 'error', 4000);
                 if (hckButton) hckButton.disabled = false;
                return; // Stop if text insertion fails
            }

            showNotification(`Concluído! (IA: ${aiScore}%)`, 'success', 0); // Persistent success message

        } catch (error) {
            console.error('[actionGenerateEssay] Erro:', error);
            showNotification(`Erro fatal: ${error.message}`, 'error', 5000);
        } finally {
            if (hckButton) hckButton.disabled = false; // Re-enable button
        }
    }
    ...
    async function actionCopyText() {
        ...
        const allTextareas = document.querySelectorAll('textarea');
        // Assume the last textarea is the main essay field
        if (allTextareas.length < 2) { // Need at least title and essay ideally
    ...
    // --- Interface do Usuário (Menu iOS Style) ---
    function loadUI() {
        if (document.getElementById('hck-menu-ios')) return; // Already loaded

        const styles = `
:root {
    --hck-menu-bg: rgba(248, 248, 248, 0.85);
    --hck-menu-blur: 12px;
    --hck-menu-shadow: 0 6px 20px rgba(0, 0, 0, 0.15);
    --hck-border-radius: 16px;
    --hck-text-color: #333;
    --hck-button-bg: rgba(255, 255, 255, 0.9);
    --hck-button-hover-bg: rgba(235, 235, 235, 0.9);
    --hck-button-active-bg: rgba(220, 220, 220, 0.9);
    --hck-accent-color: #007AFF; /* iOS Blue */
    --hck-close-color: #8e8e93; /* iOS Gray */
}
@media (prefers-color-scheme: dark) {
    :root {
        --hck-menu-bg: rgba(44, 44, 46, 0.85); /* iOS Dark Gray */
        --hck-text-color: #EAEAEA;
        --hck-button-bg: rgba(68, 68, 70, 0.9);
        --hck-button-hover-bg: rgba(88, 88, 90, 0.9);
        --hck-button-active-bg: rgba(108, 108, 110, 0.9);
        --hck-close-color: #8e8e93;
    }
}
#hck-menu-toggle-ios {
    position: fixed; bottom: 20px; right: 20px;
    background-color: var(--hck-accent-color);
    color: white;
    font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, 'Open Sans', 'Helvetica Neue', sans-serif;
    font-size: 14px; font-weight: 600;
    padding: 8px 14px; border-radius: 50px; /* Pill shape */
    cursor: pointer; box-shadow: var(--hck-menu-shadow);
    z-index: 10000;
...
#hck-menu-ios {
    position: fixed; bottom: 75px; right: 20px; /* Position above toggle */
    background-color: var(--hck-menu-bg);
    backdrop-filter: blur(var(--hck-menu-blur));
    -webkit-backdrop-filter: blur(var(--hck-menu-blur));
    border-radius: var(--hck-border-radius);
    box-shadow: var(--hck-menu-shadow);
    padding: 12px; z-index: 10000;
    font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, 'Open Sans', 'Helvetica Neue', sans-serif;
    color: var(--hck-text-color);
...
#hck-menu-ios.visible {
    opacity: 1; transform: translateY(0) scale(1);
    pointer-events: auto; /* Allow interaction when visible */
}
#hck-menu-ios-header {
    display: flex; justify-content: space-between; align-items: center;
    padding: 0 4px 4px 4px; /* Internal padding */
    border-bottom: 1px solid rgba(128, 128, 128, 0.2);
    margin-bottom: 4px;
}
#hck-menu-ios-title { font-size: 16px; font-weight: 600; }
#hck-menu-ios-close {
    font-size: 18px; font-weight: normal; color: var(--hck-close-color);
...
.hck-menu-button {
    background-color: var(--hck-button-bg); color: var(--hck-text-color);
    border: none; border-radius: 10px; /* Slightly rounded */
...
.hck-notification-ios {
    ...
    -webkit-backdrop-filter: blur(var(--hck-menu-blur));
    color: var(--hck-text-color);
    padding: 10px 20px; border-radius: var(--hck-border-radius);
    box-shadow: var(--hck-menu-shadow);
...
.hck-notification-ios.success { background-color: rgba(52, 199, 89, 0.85); color: white; font-weight: 500; } /* Green */
.hck-notification-ios.warning { background-color: rgba(255, 149, 0, 0.85); color: white; font-weight: 500; } /* Orange */
.hck-notification-ios.error { background-color: rgba(255, 59, 48, 0.85); color: white; font-weight: 500; }   /* Red */
        `;
        ...
        const menuHTML = `
            <div id="hck-menu-ios">
                <div id="hck-menu-ios-header">
                    <span id="hck-menu-ios-title">HCK Redação</span>
                    <span id="hck-menu-ios-close">✕</span>
                </div>
                <button id="hck-btn-generate" class="hck-menu-button">Gerar Redação</button>
                <button id="hck-btn-copy" class="hck-menu-button">Copiar Texto</button>
                <button id="hck-btn-clear" class="hck-menu-button">Limpar Campos</button>
            </div>
        `;
        ...
        toggleButton.addEventListener('click', () => {
            ...
            toggleButton.style.opacity = '0'; // Hide toggle button
            toggleButton.style.pointerEvents = 'none';
        });

        closeButton.addEventListener('click', () => {
            menu.classList.remove('visible');
            toggleButton.style.opacity = '1'; // Show toggle button
    ...
    // --- Inicialização ---
    // Assign actions to window scope so buttons in HTML can call them
